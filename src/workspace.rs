...
pub struct LocationInfo {
    pub path: PathBuf,
    pub kind: indexer::DefinitionKind,
    pub line: u32,
    pub col: u32,
}
...
impl WorkspaceIndex {
    ...
    pub fn index_file(&mut self, path: &Path, source: &[u8]) {
        let defs = indexer::index_source(source);
        if defs.is_empty() {
            return;
        }
    ...
    pub fn lookup_constant(&self, reference: &Reference, cursor_file: &Path) -> Vec<&LocationInfo> {
    ...
    pub fn lookup_method(&self, reference: &Reference, cursor_file: &Path) -> Vec<&LocationInfo> {
...
mod tests {
    ...
    fn lookup_method_paths(
        index: &WorkspaceIndex,
        reference: &Reference,
        cursor_file: &Path,
    ) -> Vec<PathBuf> {
        index
            .lookup_method(reference, cursor_file)
            .iter()
